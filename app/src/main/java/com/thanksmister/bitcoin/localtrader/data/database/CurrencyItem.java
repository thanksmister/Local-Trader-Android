...
public abstract class CurrencyItem implements Parcelable {
    public static final String TABLE = "currency_item";

    public static final String ID = "_id";
    public static final String CURRENCY = "currency";

    public static final String QUERY = "SELECT * FROM " + CurrencyItem.TABLE;

    public abstract long id();

    public abstract String currency();

    /*public static final Func1<Query, List<CurrencyItem>> MAP = new Func1<Query, List<CurrencyItem>>() {
        @Override
        public List<CurrencyItem> call(Query query) {
            Cursor cursor = query.run();
            try {
                List<CurrencyItem> values = new ArrayList<>(cursor.getCount());
                while (cursor.moveToNext()) {
                    long id = Db.getLong(cursor, ID);
                    String currency = Db.getString(cursor, CURRENCY);
                    values.add(new AutoParcel_CurrencyItem(id, currency));
                }
                return values;
            } finally {
                cursor.close();
