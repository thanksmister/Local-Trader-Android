...
interface LocalBitcoinsRequest {

    @GET("/api/myself/")
    fun getMyself(@Query("access_token") token:String): Observable<User>

    @GET("/api/notifications/")
    fun getNotifications(@Query("access_token") token:String): Observable<List<Notification>>

    @GET("/api/wallet-balance/")
    fun getWalletBalance(@Query("access_token") token:String): Observable<Wallet>

    @FormUrlEncoded
    @POST("/oauth2/access_token/")
    fun getOauthToken(@Field("grant_type") grant_type: String,
                      @Field("code") code: String,
                      @Field("client_id") client_id: String,
                      @Field("client_secret") client_secret: String): Observable<OauthResponse>
    ...
    fun sendPhoto(
            ...
            @Part("caption") text: RequestBody,
            @Part("photo\"; filename=\"image.jpg") photo: RequestBody
    ): Call<JSONObject>*/
}
